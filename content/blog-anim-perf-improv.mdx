...
[Chrome 공식 블로그](https://developer.chrome.com/blog/re-rastering-composite)에서 설명하고 있듯이, Chrome 53부터 `will-change: transform`이 없는 합성 레이어의 **transform scale이 변경**되면 콘텐츠를 새로운 스케일에 맞춰 다시 래스터라이징한다. 이는 확대/축소 시 텍스트나 이미지가 흐려지지 않고 선명하게 유지되도록 하기 위한 동작이다. 순수한 `translate`만 변경되는 경우에는 비트맵의 해상도가 달라질 이유가 없으므로 re-raster가 발생하지 않는다.
